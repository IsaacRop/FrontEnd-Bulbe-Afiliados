...
function criarCardPrincipal(produto) {
  ...
  // --- 4. BUSCAR PRODUTOS DO AFILIADO PELA API ---
  // 1. Busca o afiliado pelo nome para obter o ID
  api.get('/afiliados')
    .then(response => response.json())
    .then(afiliados => {
      // afiliados é array direto
      const lista = Array.isArray(afiliados) ? afiliados : (afiliados.data || []);
      const afiliado = lista.find(a => a.nome === NOME_AFILIADO);
      if (!afiliado) throw new Error('Afiliado não encontrado: ' + NOME_AFILIADO);
      return api.get('/afiliados/' + afiliado.id + '/produtos');
    })
    .then(response => response.json())
    .then(todosOsProdutos => {
      const produtosDoAfiliado = Array.isArray(todosOsProdutos) ? todosOsProdutos : (todosOsProdutos.data || []);
